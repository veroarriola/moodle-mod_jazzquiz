...
class activequiz {
    ...
    protected function add_dummy_question($questionid) {

        global $DB;

        // Create the new ActiveQuiz question
        $question = new \stdClass();
        $question->activequizid = $this->activequiz->id;
        ...
        $this->activequiz->questionorder .= ',' . $activequiz_question_id;
        $DB->update_record('activequiz', $this->activequiz);

    }

    /**
     * Adds all the dummy questions not already present in the quiz.
     */
    public function add_dummy_questions() {
        ...
        $quiz_questions = $DB->get_records('activequiz_questions', [
            'activequizid' => $this->activequiz->id
        ]);
        ...
        if (!$quiz_questions) {
            ...
            foreach ($dummy_questions as $dummy_question) {
                ...
                $this->add_dummy_question($dummy_question->questionid);
